...
if [ ! -f "$SOURCE/server.js" ]; then
  echo -e "${RED}ERROR:${NC} $SOURCE/server.js not found — has lan-origin/ been synced to $SOURCE?"
  exit 1
fi
if ! grep -q "$MARKER" "$SOURCE/server.js"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/server.js is STALE (no /orders routes)."
...
if ! grep -q "'/mockup/:id/:family/:size/:color'" "$SOURCE/server.js"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/server.js is STALE (no per-SIZE mockup route /mockup/:id/:family/:size/:color)."
  echo "       Re-sync your updated lan-origin/ to $SOURCE, then re-run."
  exit 1
fi
...
if ! grep -q "mockupView" "$SOURCE/server.js"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/server.js is STALE (no per-VIEW mockups — room07/cover)."
  echo "       Re-sync your updated lan-origin/ to $SOURCE, then re-run."
  exit 1
fi
...
if ! grep -q "cropFrameFromMatte" "$SOURCE/server.js"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/server.js is STALE (no cover frame-crop — covers would show pink/baked shadow)."
  echo "       Re-sync your updated lan-origin/ to $SOURCE, then re-run."
  exit 1
fi
...
if ! grep -q "bumpMockupVersion" "$SOURCE/server.js"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/server.js is STALE (no auto mockupVersion bump)."
  echo "       Re-sync your updated lan-origin/ to $SOURCE, then re-run."
  exit 1
fi
...
if ! grep -q "cacheFreshFor" "$SOURCE/server.js"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/server.js is STALE (no master-mtime cache self-heal — edited"
  echo "       photos won't update on already-rendered sizes)."
  echo "       Re-sync your updated lan-origin/ to $SOURCE, then re-run."
  exit 1
fi
if ! grep -q '"nodemailer"' "$SOURCE/package.json"; then
  echo -e "${YELLOW}WARNING:${NC} $SOURCE/package.json looks stale (no nodemailer) — re-sync it too."
fi
...
if ! [ -f "$SOURCE/invoice.js" ]; then
  echo -e "${RED}ERROR:${NC} $SOURCE/invoice.js missing — re-sync the FULL lan-origin/ dir, then re-run."
  exit 1
fi
if ! grep -q 'invoice.js' "$SOURCE/Dockerfile"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/Dockerfile does not COPY invoice.js — re-sync the updated Dockerfile."
  exit 1
fi
if ! grep -q '"pdfkit"' "$SOURCE/package.json"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/package.json missing pdfkit — re-sync it, then re-run (npm install adds it)."
  exit 1
fi
if ! grep -q '"adm-zip"' "$SOURCE/package.json"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/package.json missing adm-zip — re-sync it, then re-run (npm install adds it)."
  exit 1
fi
...
if ! grep -q 'fonts-noto-core' "$SOURCE/Dockerfile" || ! grep -q 'fonts-noto-cjk' "$SOURCE/Dockerfile"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/Dockerfile must install fonts-noto-core AND fonts-noto-cjk — re-sync the updated Dockerfile."
  exit 1
fi
if ! grep -q 'PAID IN FULL' "$SOURCE/invoice.js"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/invoice.js is STALE (no receipt/terms code) — re-sync the updated invoice.js."
  exit 1
fi
...
if ! [ -f "$SOURCE/poster.js" ]; then
  echo -e "${RED}ERROR:${NC} $SOURCE/poster.js missing — re-sync the FULL lan-origin/ dir, then re-run."
  exit 1
fi
if ! [ -d "$SOURCE/fonts" ] || ! [ -f "$SOURCE/fonts/IBMPlexMono-Light.ttf" ]; then
  echo -e "${RED}ERROR:${NC} $SOURCE/fonts/ missing (poster compositor fonts) — re-sync the FULL lan-origin/ dir INCLUDING the fonts/ subdir, then re-run."
  exit 1
fi
if ! grep -q '@resvg/resvg-js' "$SOURCE/package.json"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/package.json missing @resvg/resvg-js — re-sync the updated package.json, then re-run (npm install adds it)."
  exit 1
fi
if ! grep -q 'poster.js' "$SOURCE/Dockerfile" || ! grep -q 'COPY fonts' "$SOURCE/Dockerfile"; then
  echo -e "${RED}ERROR:${NC} $SOURCE/Dockerfile must COPY poster.js AND the fonts/ dir — re-sync the updated Dockerfile."
  exit 1
fi
...
if [ -n "$CID" ] && sudo docker exec "$CID" grep -q "$MARKER" server.js; then
  echo -e "==> ${GREEN}PASS${NC} — the new origin is live."
else
  echo -e "==> ${RED}FAIL${NC} — the container is NOT running the new server.js."
