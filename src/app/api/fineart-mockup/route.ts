...
export async function GET(req: Request): Promise<Response> {
  ...
  // Only ever serve/cache a real JPEG (both views are opaque JPEGs). Anything else
  // is treated as not-ready → 404 (graceful fallback) and NOT cached, so the 1-year
  // immutable cache is never poisoned.
